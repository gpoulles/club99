...
export class NumPadComponent implements OnInit, AfterViewInit {
  ...
  @ViewChildren('numButton', { read: ElementRef }) numButtons:
    | QueryList<ElementRef>
    | undefined;
  @ViewChild('backSpace', { read: ElementRef }) backSpace:
    | ElementRef
    | undefined;
  ...
  ngAfterViewInit() {
    this.renderer.listen('window', 'keydown', (event: KeyboardEvent) => {
      const matchingButton = this.numButtons
        ?.toArray()
        .find(
          (elementRef: ElementRef) =>
            elementRef.nativeElement.textContent.trim() === event.key,
        );
      ...
      if (event.key === 'Backspace') this.backSpace?.nativeElement.click();

      if (event.key === 'Enter') this.enter?.nativeElement.click();
    });
  }
}
